...
@baseline_registry.register_trainer(name="ppo_trainer")
class VLNCEPPOTrainer(BaseRLTrainer):
    ...
    def _init_train(self):
        ...
        # load from past train, legacy
        if self.config.RL.load_net:
            self.actor_critic.extra_load_net(self.config)
        if self._is_distributed:
    ...
    @rank0_only
    def _eval_log(self, writer, aggregated_stats, step_id):
        writer.add_scalar(
    ...
    def _eval_checkpoint(
        ...
        self._eval_log(writer, aggregated_stats, step_id)
        if config.EVAL.SAVE_RESULTS:
    ...
    def inference(self) -> None:
        ...
        # )
        self.policy = self.agent.actor_critic
        self.policy.eval()
        ...
        with tqdm.tqdm(
            ...
            while envs.num_envs > 0:
                current_episodes = envs.current_episodes()
                with torch.no_grad():
                    _, actions, _, rnn_states = self.policy.act(
                        batch,
                        rnn_states,
                        prev_actions,
                        not_done_masks,
