...
class EEPolicy(ILPolicy):
    ...
    def load_state_dict_woenc(self, state_dict: 'OrderedDict[str, Tensor]', strict: bool = True, excludes: list = ["clip_encoder","depth_encoder"]):
        '''Load state dict without pre-trained encoders
        '''
        state_dict_ret = self.state_dict()
        for k in state_dict.keys():
            if k.split(".")[1] not in excludes and k in state_dict_ret and state_dict_ret[k].shape==state_dict[k].shape:
...
class EENet(Net):
    def __init__(
        ...
        self.total_len = 4+self.depth_len+self.rgb_len+self.instruction_len+self.sub_len
        if model_config.EVOENC.learnable_pe:
            self.positional_embedding = nn.Parameter(torch.empty(self.total_len, self._hidden_size))
            nn.init.normal_(self.positional_embedding, std=0.01)
        else:
            self.positional_embedding = nn.Parameter(positionalencoding1d(self.total_len, self._hidden_size), requires_grad=False)
        ...
        self.transformer = Transformer(width=self._hidden_size, layers=self._transformer_layers, heads=self._transformer_heads, dropout=self._inner_dropout)
        ...
        if self.config.PRETRAIN.stage!="NONE":
            ...
            self.rgb_reconstruction = nn.Linear(self._hidden_size, self.model_config.CLIP.vit_size)
            self.depth_reconstruction = nn.Linear(self._hidden_size, self.model_config.DEPTH_ENCODER.single_size)
            self.inst_reconstruction = nn.Linear(self._hidden_size, self.model_config.CLIP.output_size)
            self.sub_reconstruction = nn.Linear(self._hidden_size, self.model_config.CLIP.output_size)
            # mean feature reconstruction
            self.mean_rgb_reconstruction = nn.Linear(self._hidden_size, self.model_config.CLIP.vit_size)
            self.mean_depth_reconstruction = nn.Linear(self._hidden_size, self.model_config.DEPTH_ENCODER.single_size)
            self.mean_inst_reconstruction = nn.Linear(self._hidden_size, self.model_config.CLIP.output_size)
            self.mean_sub_reconstruction = nn.Linear(self._hidden_size, self.model_config.CLIP.output_size)
        ...
        if model_config.EVOENC.freeze_weights>=0:
            ...
            for param in [self.positional_embedding, self.type_embedding, self.token_embedding]:
    ...
    def forward(
        ...
        if self.pe_type=="position":
            ...
            seq_embedding = seq_embedding + token_ids_embedding + self.positional_embedding.expand((seq_embedding.shape[0],-1,-1))
        ...
        mask_inst = mask_inst.unsqueeze(1).unsqueeze(1).expand(-1, self._transformer_heads, self.total_len, -1)
        mask_sub = mask_sub.unsqueeze(1).unsqueeze(1).expand(-1, self._transformer_heads, self.total_len, -1)
        # Fill
        ...
        inst_feature = seq_out[:,self.rgb_len+self.depth_len+2,:].to(dtype=rnn_states.dtype)
        sub_feature = seq_out[:,self.rgb_len+self.depth_len+self.instruction_len+3,:].to(dtype=rnn_states.dtype)
        ...
        if self.model_config.EVOENC.aggregate=="cat":
            total_feature = torch.cat([rgb_feature, depth_feature, inst_feature, sub_feature], dim=1)
    ...
    def stage0_forward(self, observations, positive=True):
        ...
        if self.pe_type=="pt":
            a = self.positional_embedding[0:self.rgb_len+1,:]
            split_position_embedding = torch.cat([a], dim=0).expand((seq_rgb_embedding.shape[0],-1,-1))
            a = self.type_embedding[0:1,:].repeat((self.rgb_len+1, 1))
            token_ids_embedding = torch.cat([a], dim=0).expand((seq_rgb_embedding.shape[0],-1,-1))
            seq_rgb_embedding = seq_rgb_embedding + split_position_embedding + token_ids_embedding
        ...
        if self.pe_type=="pt":
            b = self.positional_embedding[0:self.depth_len+1,:]
            split_position_embedding = torch.cat([b], dim=0).expand((seq_depth_embedding.shape[0],-1,-1))
            b = self.type_embedding[1:2,:].repeat((self.depth_len+1, 1))
            token_ids_embedding = torch.cat([b], dim=0).expand((seq_depth_embedding.shape[0],-1,-1))
            seq_depth_embedding = seq_depth_embedding + split_position_embedding + token_ids_embedding
        # Transformer blocks
        ...
        if self.pe_type=="pt":
            c = self.positional_embedding[0:self.instruction_len+1,:]
            split_position_embedding = torch.cat([c], dim=0).expand((seq_inst_embedding.shape[0],-1,-1))
            c = self.type_embedding[2:3,:].repeat((self.instruction_len+1, 1))
            token_ids_embedding = torch.cat([c], dim=0).expand((seq_inst_embedding.shape[0],-1,-1))
            seq_inst_embedding = seq_inst_embedding + split_position_embedding + token_ids_embedding
        start_inst = 1
        T_N = seq_inst_embedding.shape[0]
        # (T*N, heads, L, L)
        inst_len = 1+self.instruction_len
        attn_mask = torch.zeros((T_N, self._transformer_heads, inst_len, inst_len), dtype=bool).to(rgb_embedding.device)
        # Fill, (T*N, Linst) -> (T*N, heads, Linst, Linst)
        attn_mask[:, :, :, start_inst:] = mask_inst.unsqueeze(1).unsqueeze(1).expand(-1, self._transformer_heads, inst_len, -1)
        ...
        loss_rec += F.mse_loss(inst_rec, self.inst_features[feature_mask_inst])/COEF_REC_INST
        # mean feature reconstruction
        inst_mean_gt = self.inst_features.sum(dim=1)/((~mask_inst).sum(dim=1, keepdim=True)+EPS)
        ...
        if self.pe_type=="pt":
            d = self.positional_embedding[0:self.sub_len+1,:]
            split_position_embedding = torch.cat([d], dim=0).expand((seq_sub_embedding.shape[0],-1,-1))
            d = self.type_embedding[3:4,:].repeat((self.sub_len+1, 1))
            token_ids_embedding = torch.cat([d], dim=0).expand((seq_sub_embedding.shape[0],-1,-1))
            seq_sub_embedding = seq_sub_embedding + split_position_embedding + token_ids_embedding
        start_sub = 1
        T_N = seq_sub_embedding.shape[0]
        # (T*N, heads, L, L)
        sub_len = 1+self.sub_len
        attn_mask = torch.zeros((T_N, self._transformer_heads, sub_len, sub_len), dtype=bool).to(rgb_embedding.device)
        # Fill, (T*N, Linst) -> (T*N, heads, Linst, Linst)
        attn_mask[:, :, :, start_sub:] = mask_sub.unsqueeze(1).unsqueeze(1).expand(-1, self._transformer_heads, sub_len, -1)
        ...
        # mean feature reconstruction
        sub_mean_gt = self.sub_features.sum(dim=1)/((~mask_sub).sum(dim=1, keepdim=True)+EPS)
    ...
    def stage1_forward(self, observations):
        ...
        self.depth_seq_features = depth_embedding_seq[positive_idx].detach().clone() # self.depth_encoder.get_depth_seq_features()
        ...
        if self.pe_type=="pt":
            a = self.positional_embedding[0:self.rgb_len+1,:]
            b = self.positional_embedding[0:self.depth_len+1,:]
            split_position_embedding = torch.cat([a,b], dim=0).expand((seq_vision_embedding.shape[0],-1,-1))
            a = self.type_embedding[0:1,:].repeat((self.rgb_len+1, 1))
            b = self.type_embedding[1:2,:].repeat((self.depth_len+1, 1))
            token_ids_embedding = torch.cat([a,b], dim=0).expand((seq_vision_embedding.shape[0],-1,-1))
            seq_vision_embedding = seq_vision_embedding + split_position_embedding + token_ids_embedding
        # Transformer blocks
        ...
        loss_align += F.binary_cross_entropy(torch.sigmoid(align_pre_v.squeeze()), align_gt.float())
        ...
        if self.pe_type=="pt":
            c = self.positional_embedding[0:self.instruction_len+1,:]
            d = self.positional_embedding[0:self.sub_len+1,:]
            split_position_embedding = torch.cat([c,d], dim=0).expand((seq_language_embedding.shape[0],-1,-1))
            c = self.type_embedding[2:3,:].repeat((self.instruction_len+1, 1))
            d = self.type_embedding[3:4,:].repeat((self.sub_len+1, 1))
            token_ids_embedding = torch.cat([c,d], dim=0).expand((seq_language_embedding.shape[0],-1,-1))
            seq_language_embedding = seq_language_embedding + split_position_embedding + token_ids_embedding
        start_inst = 1
        start_sub = 2+self.instruction_len
        T_N = seq_language_embedding.shape[0]
        # (T*N, heads, L, L)
        language_len = 2+self.instruction_len+self.sub_len
        attn_mask = torch.zeros((T_N, self._transformer_heads, language_len, language_len), dtype=bool).to(rgb_embedding.device)
        # Fill, (T*N, Linst) -> (T*N, heads, Linst, Linst)
        attn_mask[:, :, :, start_inst:start_sub-1] = mask_inst.unsqueeze(1).unsqueeze(1).expand(-1, self._transformer_heads, language_len, -1)
        attn_mask[:, :, :, start_sub:] = mask_sub.unsqueeze(1).unsqueeze(1).expand(-1, self._transformer_heads, language_len, -1)
        ...
        loss_rec += F.mse_loss(sub_rec, self.sub_features[feature_mask_sub])
        # mean feature reconstruction
        inst_mean_gt = self.inst_features.sum(dim=1)/((~(mask_inst[positive_idx])).sum(dim=1, keepdim=True)+EPS)
        inst_mean_rec = self.mean_inst_reconstruction(inst_cls[positive_idx])
        loss_mean += F.mse_loss(inst_mean_rec, inst_mean_gt)
        sub_mean_gt = self.sub_features.sum(dim=1)/((~(mask_sub[positive_idx])).sum(dim=1, keepdim=True)+EPS)
        ...
        loss_align += F.binary_cross_entropy(torch.sigmoid(align_pre_l.squeeze()), align_gt.float())
    ...
    def stage2_forward(self, observations):
        ...
        rgb_out = seq_out[positive_idx,1:self.rgb_len+1,]
        depth_out = seq_out[positive_idx,self.rgb_len+2:self.rgb_len+self.depth_len+2,:]
        ...
        rgb_depth_cls = torch.cat([rgb_cls,depth_cls],dim=1)
        align_pre_v = self.inner_alignment(rgb_depth_cls)
        loss_inner += F.binary_cross_entropy(torch.sigmoid(align_pre_v.squeeze()), inner_gt.float())
        inst_sub_cls = torch.cat([inst_cls,sub_cls],dim=1)
        align_pre_l = self.inner_alignment(inst_sub_cls)
        loss_inner += F.binary_cross_entropy(torch.sigmoid(align_pre_l.squeeze()), inner_gt.float())

        # outer alignment, all samples are involved
        rgb_depth_inst_sub_cls = torch.cat([rgb_cls,depth_cls,inst_cls,sub_cls],dim=1)
        align_pre = self.outer_alignment(rgb_depth_inst_sub_cls)
        loss_outer += F.binary_cross_entropy(torch.sigmoid(align_pre.squeeze()), outer_gt.float())
